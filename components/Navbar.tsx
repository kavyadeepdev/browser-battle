...
const navItems = [
  ...
  },
  {
    ...
    links: [
      ...
      { label: 'Mechanical Engineering', href: '/academics#mech' },
      { label: 'AI & ML', href: '/academics#aiml' },
      { label: 'Aerospace', href: '/academics#aero' },
      { label: 'Bio-Technology', href: '/academics#biotech' },
  ...
  },
  {
    ...
    links: [
      ...
      { label: 'Syllabus', href: '/admissions#syllabus' }
    ]
  },
  {
    label: 'Research',
      ...
      <div className="max-w-[1440px] mx-auto px-10 h-full flex items-center">
        ...
        <div className="flex-1 flex justify-start">
          <Link href="/" className="flex items-center space-x-3 group">
            <div className="flex flex-col">
              <span className="font-serif font-black text-cream text-xl leading-none tracking-tighter uppercase group-hover:text-gold transition-colors duration-500">BMSCE</span>
              <span className="font-serif text-[8px] text-gold uppercase tracking-[0.5em] font-bold mt-1 opacity-80">Bengaluru</span>
            </div>
          </Link>
        </div>
        ...
        <div className="hidden lg:flex items-center justify-center space-x-10">
          {navItems.map((item) => (
            ...
            >
              <Link href={item.href} className="flex items-center space-x-1 font-sans text-[11px] font-medium text-cream/70 hover:text-cream transition-all duration-500 uppercase tracking-[0.2em]">
        ...
        <div className="flex-1 hidden lg:flex items-center justify-end space-x-10">
          <div className="flex items-center space-x-10 border-r border-white/10 pr-10">
            <a href="#" className="text-cream/40 hover:text-gold transition-colors duration-500 font-sans text-[9px] uppercase tracking-[0.2em] font-bold">Login</a>
            <a href="#" className="text-cream/40 hover:text-gold transition-colors duration-500 font-sans text-[9px] uppercase tracking-[0.2em] font-bold">Results</a>
            <a href="#" className="text-cream/40 hover:text-gold transition-colors duration-500 font-sans text-[9px] uppercase tracking-[0.2em] font-bold">Contact</a>
          </div>
          <button className="text-cream/40 hover:text-gold transition-colors duration-500">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
          </button>
          <Link href="/admissions" className="border border-white/20 text-cream px-6 py-2 font-sans text-[10px] font-bold uppercase tracking-[0.2em] hover:bg-cream hover:text-navy-deep transition-all duration-500 rounded-full">
            Apply Now
          </Link>
        </div>
            ...
            <div className="px-10 py-10 space-y-8">
              {navItems.map((item) => (
                <div key={item.label} className="space-y-4">
                  <Link href={item.href} className="font-sans font-bold text-gold text-xs uppercase tracking-[0.3em]">{item.label}</Link>
                  <div className="grid grid-cols-1 gap-3">
                    {item.links.map((link) => (
                      <Link key={link.label} href={link.href} className="text-[11px] font-sans text-cream/50 hover:text-cream transition-colors">
                        {link.label}
                      </Link>
                    ))}
                  </div>
              ...
              <Link href="/admissions" className="block text-center w-full border border-white/20 text-cream rounded-full py-4 font-sans text-[10px] uppercase tracking-[0.2em] font-bold">
                Apply Now
              </Link>
            </div>
          </motion.div>
